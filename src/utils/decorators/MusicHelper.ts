...
export function isMusicPlaying(): any {
    return inhibit(message => {
        if (message.guild?.queue === null) return message.channel.send(new MessageEmbed().setDescription("There is nothing playing.").setColor("#FFFF00"));
    });
}
...
export function isUserInTheVoiceChannel(): any {
    return inhibit(message => {
        if (!message.member?.voice.channel) {
            return message.channel.send(
                new MessageEmbed().setDescription("I'm sorry but you need to be in a voice channel to do that").setColor("#FFFF00")
            );
        }
    });
}
