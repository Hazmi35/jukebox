...
export class ServerQueue {
    ...
    public async play(guild: Guild): Promise<any> {
        ...
        entersState(serverQueue.connection!, VoiceConnectionStatus.Ready, 15 * 1000)
            .then(() => serverQueue.player.play(song))
            .catch(e => {
                if (e.message === "The operation was aborted") e.message = "Could not establish a voice connection within 15 seconds.";
                serverQueue.player.emit("error", new AudioPlayerError(e, song));
            });

        serverQueue.player.on("stateChange", (oldState, newState) => {
            if (newState.status === AudioPlayerStatus.Playing) {
                if (oldState.status === AudioPlayerStatus.Paused) return undefined;
                this.client.logger.info(`${this.client.shard ? `[Shard #${this.client.shard.ids[0]}]` : ""} Track: "${song.metadata.title}" on ${guild.name} started`);
                serverQueue.textChannel?.send({ embeds: [createEmbed("info", `▶ Start playing: **[${song.metadata.title}](${song.metadata.url})**`).setThumbnail(song.metadata.thumbnail)] })
                    .then(m => serverQueue.oldMusicMessage = m.id)
                    .catch(e => this.client.logger.error("PLAY_ERR:", e));
                return undefined;
            }
            if (newState.status === AudioPlayerStatus.Idle) {
                this.client.logger.info(`${this.client.shard ? `[Shard #${this.client.shard.ids[0]}]` : ""} Track: "${song.metadata.title}" on ${guild.name} ended`);
                ...
                serverQueue.textChannel?.send({ embeds: [createEmbed("info", `⏹ Stop playing: **[${song.metadata.title}](${song.metadata.url})**`).setThumbnail(song.metadata.thumbnail)] })
                    .then(m => serverQueue.oldMusicMessage = m.id)
                    .catch(e => this.client.logger.error("PLAY_ERR:", e))
                    .finally(() => {
